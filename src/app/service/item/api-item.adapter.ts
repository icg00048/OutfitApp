...
const mapCategory = (c?: BackendCategory): Category => {
  switch (c) {
    ...
    case 'FOOTWEAR': return Category.Shoes;
...
export function toFrontItem(b: BackendItem): FrontItem {
  ...
  const primaryColor = normalizeColor(d['primary_color'] ?? b.color);
  const secondaryColorRaw = d['secondary_color'];
  const secondaryColor =
    secondaryColorRaw && secondaryColorRaw !== 'none'
      ? normalizeColor(secondaryColorRaw)
      : null;
